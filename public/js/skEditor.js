    ...
    $.skEditor = function(box, options) {
        ...
        var group3 = '<div class="sk-btn-group">\
                ...
                <div class="sk-btn dropdown">\
                    ...
                    <button type="button" class="sk dropdown-toggle" id="dropdownMenuColor'+id+'" data-bs-toggle="dropdown" aria-expanded="false">\
                ...
                <div class="sk-btn dropdown">\
                    ...
                    <button type="button" class="sk dropdown-toggle" id="dropdownMenuBgColor'+id+'" data-bs-toggle="dropdown" aria-expanded="false">\
                        <span class="sr-only">Toggle Dropdown</span>\
                    </button>\
        ...
        var group5 = '<div class="sk-btn-group">\
                <div class="sk-btn dropdown">\
                    <button type="button" class="sk list dropdown-toggle" id="dropdownMenuBullet'+id+'" data-bs-toggle="dropdown" aria-expanded="false" title="Bullet list"><i class="fas fa-list-ul fs-17"></i></button>\
        ...
        AColorPicker.from(area.find('.color-picker')).on('change',(picker,color)=>{area.find('.sk-preview').css('background-color',color);
        document.getSelection();document.execCommand('foreColor',false,color)});
        AColorPicker.from(area.find('.background-color-picker')).on('change',(picker,color)=>{
            document.getSelection();document.execCommand('backColor',false,color)
        ...
        area.find('.sk.no-background').click(function(){
            document.getSelection();document.execCommand('backColor', false, 'unset')
            ...
            ImageModal.on('click','.im-select',function(){
                ...
                if($(this).closest('.row').hasClass('im-s')) {
                    ...
                    required.map(function(k,v){ 
                        // console.log(v,k)
                        if(v.value == ''){ v.classList.add('is-invalid'); error.push(v.getAttribute('name')) }
                        else{ v.classList.remove('is-invalid'); delete error[v.getAttribute('name')]; }
                    })
                    if(error.length < 1){
